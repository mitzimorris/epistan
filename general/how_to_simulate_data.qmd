...
These models can be used for any observational dataset consiting of a per-datum
...
thus demonstrating the range of behavoirs compatible with a single draw or summary from the fitted model.
...
Since these assumptions are rarely, if ever met, a better model would account for
non-homogenous populations, noisy tests, and what is already known about the disease.


**Always Start with "Hello, World!"**

It's tempting to start by refining the above model to address these obvious deficits - *please don't!* </br>
...
Furthermore, in the Bayesian workflow,  this simple Bernoulli model provides a baseline.
...
hello_data = hello_datagen.sample(data={"N":100}, chains=1, iter_warmup=0, iter_sampling=10)
```
...
This is model `binomial_model_p0_g2.stan`.
[The suffix `p0_g2` is shorthand for "0 population-level effects,
2 group-level effects", in an attempt to encode model structure information
in the filename. For this case study, and in the Bayesian Workflow
in general, choosing good names for alternative models is challenging.]{.aside}
...
The model's data inputs consist of

* Data size and shape: how many observations, how many categories for age and ethnicity?

* The observations: demographic data, test outcomes.

We need to know the number of age, ethnicity categories before
choosing the regression co-efficients for each.
We also want to be able to specify the total number of observations
so that we can explore model fits for different data regimes.

For this version of the model, 
we assume an equal number of observations for each population demographic
and that all demographics are observed.
Therefore the total number of strata `N` is the cross-product of `N_age` and `N_eth`.
This computation must done in the `transformed data` block
...
in file `binomial_datagen_p0_g2.stan`:
...
As above, we compile the program `binomial_datagen_p0_g2.stan`,
specify the size and shape of the generated dataset,
and run the model for 10 iterations using a single chain.

```{python}
#| output: false
binom_datagen_p0_g2 = CmdStanModel(stan_file='binomial_datagen_p0_g2.stan')
datagen_size_shape = {'N_obs':3300, 'N_age':11, 'N_eth':3}
data_p0_g2 = binom_datagen_p0_g2.sample(data=datagen_size_shape, chains=1, iter_warmup=0, iter_sampling=10)
```
...
```{python}
for i in range(10):
    print('alpha', data_p0_g2.alpha[i])
    print('sigma_age', data_p0_g2.sigma_age[i], 'beta_age', data_p0_g2.beta_age[i])
    print('sigma_eth', data_p0_g2.sigma_eth[i], 'beta_eth', data_p0_g2.beta_eth[i])
    print('positive tests: ', data_p0_g2.pos_tests[i].astype(int), '\n')
```
...
```{python}
sim_data = {'N': data_p0_g2.tests[0].shape[0],
            'N_age': data_p0_g2.beta_age[0].shape[0],
            'N_eth': data_p0_g2.beta_eth[0].shape[0],
    	    'age': data_p0_g2.age[0].astype(int),
    	    'eth': data_p0_g2.eth[0].astype(int),
            'tests': data_p0_g2.tests[0].astype(int),
    	    'pos_tests': data_p0_g2.pos_tests[0].astype(int)}
```

We compile and run the binomial model.

```{python}
binom_p0_g2 = CmdStanModel(stan_file='binomial_model_p0_g2.stan')
binom_p0_g2_fit = binom_p0_g2.sample(data=sim_data)
binom_p0_g2_fit.summary()
```
...
this is model `binomial_model_p1_g2.stan`.
...
**Exercise:**   Write the corresponding data generating program:  `binomial_datagen_p1_g2.stan`.
