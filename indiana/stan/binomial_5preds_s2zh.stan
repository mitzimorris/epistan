data {
  ...
  int<lower=2> N_age, N_eth, N_time, N_zip;
  ...
  array[N] int<lower=1, upper=N_zip> zip;
...
parameters {
  ...
  real<lower=0> sigma_age, sigma_eth, sigma_time, sigma_zip;
  vector[N_age - 1] beta_age_raw;
  vector[N_eth - 1] beta_eth_raw;
  vector[N_time - 1] beta_time_raw;
  vector[N_zip - 1] beta_zip_raw;
}
transformed parameters {
  ...
  vector<multiplier=sigma_time>[N_time] beta_time = append_row(beta_time_raw, -sum(beta_time_raw));
  vector<multiplier=sigma_zip>[N_zip] beta_zip = append_row(beta_zip_raw, -sum(beta_zip_raw));
  vector[N] pop_prev =  inv_logit(alpha + beta_sex[sex] + beta_age[age] + beta_eth[eth] + beta_time[time] + beta_zip[zip]);
...
model {
  ...
  beta_time ~ normal(0, sigma_time);
  beta_zip ~ normal(0, sigma_zip);

  sigma_age ~ normal(0, 2.5);
  sigma_eth ~ normal(0, 2.5);
  sigma_time ~ normal(0, 2.5);
  sigma_zip ~ normal(0, 2.5);
}
