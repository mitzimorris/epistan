...
foo = anon_data_munged %>% group_by(eth,age,sex,time) %>% summarise(pos_tests = sum(y), .groups="keep") %>% ungroup()
bar = anon_data_munged %>% group_by(eth,age,sex,time) %>% summarise(tests = n(), .groups="keep") %>% ungroup()
foo[] <- lapply(foo, function(x) if(is.factor(x)) as.integer(x) else x)
strata = foo
strata$tests = bar$tests

write.csv(strata, "data_tests_sex_age_eth_time.csv", row.names=FALSE)
